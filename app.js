...
function initDashboardApp() {
  ...
  function normalizeRecord(d, idx) {
    let role = (d.role || d.Role || d['Specialist Role'] || '').trim();
    if (/^rave programmer$/i.test(role)) role = 'RAVE Programmer';
    if (!role) role = 'CDM Specialist';

    let name = (d.name || d.Name || d['Candidate Name'] || `Candidate ${idx + 1}`).trim();
    let fn = (d.function || d.Function || d['Functional Domain'] || 'CDM').trim();
    let l1Date = (d.interviewDate || d['Interview Date'] || d['L1 Date'] || d['Round 1'] || '').trim();
    let l2 = (d.interview2 || d['Interview 2'] || d['Round 2'] || d['L2 Round 2'] || '').trim();
    let fb = (d.clientFeedback || d['Client Feedback'] || d.Feedback || '').trim();
    let st = (d.status || d.Status || fb || 'Pipeline').trim();
    let pCtc = (d.presentCtcRaw || d['Present CTC'] || d.presentCtc || '').trim();
    let oCtc = (d.offeredCtcRaw || d['Offered CTC'] || d.offeredCtc || '').trim();
    let doj = (d.doj || d.DOJ || d['Date of Joining'] || '').trim();
    let ob = (d.onboard || d.Onboard || d.Onboarding || '').trim();
    let skill = (d.skillGroup || d['Skill Group'] || 'CDM').trim();

    let city = (d.currentLocation || d.city || d.City || d.Location || 'Bangalore Hub').trim();
    let notice = (d.noticePeriod || d.Notice || d['Notice Period'] || '30 Days Notice').trim();
    let exp = d.experienceYears || d.Exp || d.Experience || '6.0';

    return {
      sno: d.sno || d['S.No'] || d['SNo'] || (idx + 1),
      id: d.id || `CDM-${String(idx + 1).padStart(3, '0')}`,
      name,
      function: fn,
      role,
  ...
  function renderDashTrend(tc) {
    ...
    const runwaysHtml = ROLE_TIMELINES.map(item => {
      const color = ROLE_COLORS[item.role] || PALETTE.amethyst;
    ...
    reader.onload = e => {
      try {
        ...
        if (json && json.length > 0) {
          ...
          if (hint) hint.innerHTML = `<strong style="color:var(--clr-emerald)">✓ Loaded ${json.length} candidate rows from ${file.name}</strong>`;
        } else {
          alert('Could not detect candidate rows in file. Please ensure columns exist.');
        }
      } catch (err) {
        console.error(err);
